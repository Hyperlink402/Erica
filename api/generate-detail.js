export default async function handler(req, res) {
  ...
  const lockKey = `generate-detail-inflight:${id}`;
  let acquiredLock = false;

  try {
    if (UPSTASH_URL && UPSTASH_TOKEN) {
      acquiredLock = await tryAcquireLock(lockKey, 25);
      if (!acquiredLock) {
        const retryAfterSeconds = 3;
        res.setHeader('Retry-After', String(retryAfterSeconds));
        return res.status(429).json({ error: '해당 기사의 요약이 이미 처리 중입니다. 잠시 후 다시 시도해 주세요.' });
      }
